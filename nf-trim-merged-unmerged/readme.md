# nf-trim-merged-unmerged: Multi-Era Ancient & Modern DNA Mapping Pipeline

## Overview
This pipeline is a Nextflow DSL2 workflow designed for processing high-throughput sequencing data across dual temporal eras (**modern** and **historical/ancient** cohorts). Built to runn on Old Dominion University's WAHAB cluster. It does masking of repeats in the reference genome, adapter trimming, overlapping read merging, length normalization,  mapping, and optional fastqc quality evaluation, AMBER evaluation, and mapdamage rescaling of historical bam files.

The pipeline automatically maps historical samples with a user-chosen algorith to extract their average mapped read length distribution, uses this value to trim modern reads (preventing temporal length biases), and selects an alignment algorithm (`bwa aln` vs `bwa mem`) on the fly. Re-maps the historical reads if the mapper is different than initially done.
