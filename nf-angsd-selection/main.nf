...
params.species     = "Sor"   // 3-letter species code for output file naming. used in ANGSD output files and ACER results.
...
params.max_kb_dist = 50      // Maximum pairwise distance in kb to test for LD if pruning (default 50)
params.min_weight  = 0.2     // Minimum r2 threshold for pruning filter (default 0.2)
params.run_selection = true  // Set to false to skip ACER selection scan (default true)
...
params.n_boot      = 1000    // Number of bootstrap replicates to run for Ne confidence interval calculations (default 1000)
...
workflow {
    ...
    // only consider the sites that pass the depth and minind QC filters in both historical and modern populations (i.e. the intersected site set)
