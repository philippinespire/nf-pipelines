nextflow.enable.dsl=2

// Default Parameters
params.samplesheet = "${projectDir}/inputfiles/samplesheet.csv"
params.contigs     = "${projectDir}/data/reference/reference.contigs.txt"
params.outdir      = "${projectDir}/results"
params.reference   = "${projectDir}/data/reference/reference.fasta"
params.bed_file    = "${projectDir}/data/reference/reference.repma.angsd.txt" // ANGSD-style bed file that has been repeat-masked, if available. To create from a classic bed file: awk '{print $1"\t"$2+1"\t"$3}' reference.repma.bed > reference.repma.angsd.txt
params.species     = "Sor"
params.min_ind_frac   = 0.70  // Minimum 70% of individuals required per site
params.max_depth_mult = 10    // Max depth threshold = 10x average total expected coverage
params.ld_prune    = true   // Set to true to enable LD pruning
params.max_kb_dist = 50     // Maximum pairwise distance in kb to test for LD if pruning
params.min_weight  = 0.2    // Minimum r2 threshold for pruning filter
params.run_selection = true  // Set to false to skip ACER by default
params.generations = 114    // Number of generations to use for Ne calculations. Default is 114 for Albatross to contemporary populations with a 1 year generation time. Adjust for other species as needed.
params.fdr_cutoff  = 0.05   // FDR cutoff for iterative selection scan (default 0.05)
params.max_rounds  = 20     // Maximum number of rounds to run for iterative selection scan (default 20)
params.n_boot      = 1000   // Number of bootstrap replicates to run for Ne calculations (default 1000)
params.min_ind     = 4      // Minimum number of individuals to include in Ne calculations. Default is 4 for Albatross populations. Adjust for as needed.

// import modules
include { ANGSD_GL_ALL; ANGSD_COLLECT_OUTPUT; ANGSD_EXTRACT_SITES; INDEX_BED_SITES  } from './modules/angsd_gl.nf'
include { COLLECT_BAM_ALL; COLLECT_BAM_POP                                          } from './modules/collect_bam.nf'
include { PCANGSD; PLOT_PCANGSD; PLOT_ADMIXTURE                                     } from './modules/pcangsd.nf'
include { QC_POP_SITES; INTERSECT_POP_SITES; ANGSD_GL_POP                           } from './modules/angsd_gl_pop.nf'
include { ANGSD_DIVERSITY                                                           } from './modules/angsd_diversity.nf'
include { LD_PRUNE_CONTIG; MERGE_PRUNED_SITES; SUBSET_BEAGLE                        } from './modules/ld_prune.nf'
include { PREPARE_SITE_SETS                                                         } from './modules/filter_sites.nf'
include { RUN_ACER                                                                  } from './modules/acer.nf'
include { ANGSD_FST                                                                 } from './modules/angsd_fst.nf'

// Workflow 
workflow {
    // --- Input Channels (Defined inside workflow for DSL2 scope compliance) ---
    samples = channel
        .fromPath(params.samplesheet, checkIfExists: true)
        .splitCsv(header: true)
        .map { row -> tuple(row.sample, row.pop, row.era, row.region, row.bam) }

    all_bams = samples
        .map { sample, pop, era, region, bam -> bam }
        .collect()

    contigs = channel
        .fromPath(params.contigs, checkIfExists: true)
        .splitText()
        .map { it.trim() }
        .filter { it }

    pop_bams = samples
        .groupTuple(by: [1, 2])
        .map { sample_list, pop, era, regions, bams -> tuple(pop, era, bams) }

    bamlist = COLLECT_BAM_ALL(all_bams)
    bamlist_pop = COLLECT_BAM_POP(pop_bams)

    // 1. Index full callable bed file for full-genome diversity calculations (no SNP_pval filter)
    all_sites = INDEX_BED_SITES(params.bed_file)

    // 2. Run per-population QC to identify loci passing minind_frac and maxdepth_mult in historical and modern subsets
    ch_bed_bundle = all_sites.snps.combine(all_sites.bin).combine(all_sites.idx)
    qc_passing_sites = QC_POP_SITES(bamlist_pop, ch_bed_bundle)

    // 3. Intersect site lists across historical and modern populations
    intersected_sites = INTERSECT_POP_SITES(qc_passing_sites.passing_pos.collect())

    // 4. Pass 3: Generate SAF and MAF files for historical and modern populations
    // using ONLY the shared intersected site set
    ANGSD_GL_POP(
        bamlist_pop,
        intersected_sites.pos,
        intersected_sites.bin,
        intersected_sites.idx,
        all_sites.regions
    )
    
    // 5. Run ANGSD_GL_ALL to identify SNPs (uses SNP_pval 1e-6)
    genotypes = ANGSD_GL_ALL(contigs, bamlist)

    mafs = genotypes.mafs.map { contig, file -> file }.collect()
    beagle = genotypes.beagle.map { contig, file -> file }.collect()

    collected = ANGSD_COLLECT_OUTPUT(mafs, beagle)
    sites = ANGSD_EXTRACT_SITES(collected.all_mafs)

    // 6. LD Pruning (optional)
    def ch_pruned_pos = channel.value([])
    if (params.ld_prune) {
        contig_ld_input     = genotypes.beagle.join(genotypes.mafs)
        pruned_contig_sites = LD_PRUNE_CONTIG(contig_ld_input)
        merged_sites        = MERGE_PRUNED_SITES(pruned_contig_sites.collect())
        ch_pruned_pos       = merged_sites
    }

    // 7. ACER Chi-Squared selection scan (optional)
    def ch_chisq_results = channel.value([])
    if (params.run_selection) {
        RUN_ACER(
            ANGSD_GL_POP.out.mafs.collect(),
            sites.snps,
            file("${projectDir}/scripts/acer_helpers.R"),
            file("${projectDir}/scripts/run_acer.R"),
            file(params.samplesheet)
        )
        ch_chisq_results = RUN_ACER.out.chisq_results
    }

     // 8. Generate Site Set Libraries for downstream uses
    PREPARE_SITE_SETS(
        intersected_sites.pos,
        sites.snps,
        ch_pruned_pos,
        ch_chisq_results
    )

    // 9. PCA & Admixture (Runs on LD-pruned neutral SNPs)
    subset_beagle   = SUBSET_BEAGLE(collected.all_beagle, PREPARE_SITE_SETS.out.snps_pruned_neutral_pos)
    pcangsd_results = PCANGSD(subset_beagle.pruned_beagle)
    PLOT_PCANGSD(pcangsd_results.pcangsd_cov, file(params.samplesheet))
    PLOT_ADMIXTURE(pcangsd_results.q_matrix, file(params.samplesheet))

    // 10. Diversity Calculations (Runs on all callable neutral loci)
    ch_diversity_bundle = PREPARE_SITE_SETS.out.callable_neutral_pos
        .combine(PREPARE_SITE_SETS.out.callable_neutral_bin)
        .combine(PREPARE_SITE_SETS.out.callable_neutral_idx)
    ANGSD_DIVERSITY(ANGSD_GL_POP.out.saf_files, ch_diversity_bundle)

    // 11. Calc historic-modern FST
    // Split SAF channel into historical and modern channels
    ch_saf_hist = ANGSD_GL_POP.out.saf_files
        .filter { pop, era, saf, idx, pos -> era.toLowerCase() in ['historic', 'historical'] }
        .map { pop, era, saf, idx, pos -> tuple(pop, saf, idx, pos) }

    ch_saf_mod = ANGSD_GL_POP.out.saf_files
        .filter { pop, era, saf, idx, pos -> era.toLowerCase() in ['modern', 'mod'] }
        .map { pop, era, saf, idx, pos -> tuple(pop, saf, idx, pos) }

    // Join on population key (pop)
    ch_fst_pairs = ch_saf_hist.join(ch_saf_mod)
    // Structure: tuple(pop, hist_saf, hist_idx, hist_pos, mod_saf, mod_idx, mod_pos)

    // Bundle pruned neutral site files
    ch_pruned_neutral_bundle = PREPARE_SITE_SETS.out.snps_pruned_neutral_pos
        .combine(PREPARE_SITE_SETS.out.snps_pruned_neutral_bin)
        .combine(PREPARE_SITE_SETS.out.snps_pruned_neutral_idx)

    // Execute FST calculation
    ANGSD_FST(ch_fst_pairs, ch_pruned_neutral_bundle)
}